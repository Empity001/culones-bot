...
export function renderLogsImage(logs, serverName = 'Culones RPG') {
  ...
  for (const log of logs) {
    ...
    // Barra de color de relevancia a la izquierda
    ...
    // Fecha y likes a la derecha
    ctx.fillStyle = INK_600;
    ctx.font = '10.5px sans-serif';
    ctx.textAlign = 'right';
    const dateStr = new Date(log.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' });
    ctx.fillText(dateStr, CANVAS_W - PADDING - 12, y + 9);
    ctx.fillStyle = MAGENTA;
    ctx.fillText(`❤️ ${log.likes ?? 0}`, CANVAS_W - PADDING - 12, y + 31);
    ctx.textAlign = 'left';
  ...
  ctx.beginPath(); ctx.moveTo(0, footerY); ctx.lineTo(CANVAS_W, footerY); ctx.stroke();
  ctx.fillStyle = INK_600;
  ctx.font = '10px sans-serif';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(`${serverName} · ${new Date().toLocaleDateString('es-ES')}`, PADDING, footerY + FOOTER_H / 2);
  ctx.fillStyle = CYAN;
  ctx.textAlign = 'right';
